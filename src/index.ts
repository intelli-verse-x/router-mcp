...
async function call(
  base: string,
  path: string,
  opts: { method?: string; bearer: string; body?: Json; query?: Record<string, string | undefined> }
): Promise<Json> {
  if (!opts.bearer) {
    throw new Error(
...
function workspace(explicit?: string): string {
  const ws = explicit ?? WORKSPACE_ID;
  if (!ws) throw new Error("Provide workspace_id or set ROUTER_WORKSPACE_ID.");
  return ws;
}
...
server.registerTool(
  "chat",
  {
    ...
    description:
      "Send a chat completion. Never name an LLM — omit `model` (the Intelliverse default routes at " +
  ...
  async ({ prompt, system, model }) => {
    ...
    const content = (res as any)?.choices?.[0]?.message?.content ?? res;
    return ok(content);
  }
);
...
server.registerTool(
  "get_credits",
  {
    ...
    description:
      "LLM daily budget for the API key plus experience-scoped media credit wallets (image, video, voice, audio, audiobook, 3D).",
...
server.registerTool(
  "create_app",
  {
    ...
    description:
      "Create an experience with isolated Knowledge, Memory, credentials, wallet, and usage tracking. " +
...
server.registerTool(
  "create_api_key",
  {
    ...
    description:
      "Mint a tier API key (air | pro | pro_max), optionally bound to an Experience ID and daily spend limit. " +
...
server.registerTool(
  ...
  async ({ app_id, documents }) =>
    ok(await call(KB, "/v1/kb/ingest", { bearer: API_KEY, body: { app_id, documents } }))
);
...
server.registerTool(
  ...
  async ({ app_id, query, top_k }) =>
    ok(await call(KB, "/v1/kb/search", { bearer: API_KEY, body: { app_id, query, top_k } }))
);
...
server.registerTool(
  "generate_media",
  {
    ...
    description:
      "Generate image | video | voiceover | audio (music/sfx) | gen3d assets. Credits are held from the " +
  ...
  async ({ kind, app_id, prompt, text, quality_class, audio_kind }) => {
    const body: Json = { app_id, quality_class };
    if (kind === "voiceover") body.text = text ?? prompt;
    else body.prompt = prompt;
    if (kind === "audio") body.kind = audio_kind ?? "music";
